...
//  wifi credentials
const char ssid[] = "MY_WIFI0-0";
const char password[] = "PRIVATEPASSWORD";
...
Adafruit_MQTT_Publish ct = Adafruit_MQTT_Publish(&mqtt , IO_USERNAME"/feeds/temperature celcius");
Adafruit_MQTT_Publish ft = Adafruit_MQTT_Publish(&mqtt , IO_USERNAME"/feeds/temperature fahrenheit");
Adafruit_MQTT_Publish kt = Adafruit_MQTT_Publish(&mqtt , IO_USERNAME"/feeds/temperature kelvin");
Adafruit_MQTT_Publish hum = Adafruit_MQTT_Publish(&mqtt , IO_USERNAME"/feeds/humidity");
...
void loop()
{
  ...
  if (!ct.publish(tempc)  ||  !ft.publish(tempf)  ||  !kt.publish(tempk)  ||  !dp.publish(dew_point)  ||  !hum.publish(humidity))
  {
  ...
  //  only this type of variable/object can cakth feed and then we can extract its value.
